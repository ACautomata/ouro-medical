...
def precompute_freqs_cis_2d(dim: int, height: int, width: int, theta: float = 10000.0, scale=16.0):
    """
    Precompute 2D complex frequency positions for positional encoding.
    """
...
class NerfEmbedder(nn.Module):
    """
    NeRF-style positional embedding for patch inputs.
    """
    def __init__(self, in_channels, hidden_size_input, max_freqs):
        ...
        self.embedder = nn.Sequential(
            nn.Linear(in_channels + max_freqs ** 2 * 2, hidden_size_input, bias=True),
        )
...
class SimpleMLPAdaLN(nn.Module):
    """
    The MLP for Diffusion Loss with adaptive layer normalization (adaLN).
    ...
    :param patch_size: size of patches for conditioning embedding.
    """
    ...
    def __init__(
        ...
        # Conditioning embedding: z_channels -> patch_size^2 * model_channels
...
class PositionEmbedding(nn.Module):
    """
    Learnable 2D sin-cos positional embeddings for patches.
    """
...
class PostConvSmoother(nn.Module):
    """
    Post-processing convolution blocks for artifact reduction.
    """
...
class ProgressiveConvDecoder(nn.Module):
    """
    Progressive upsampling decoder: H/32 -> H/16 -> H/8 -> H/4 -> H/2 -> H
    """
...
class PatchDecoder_postps(nn.Module):
    """
    Pixel shuffle decoder: H/32 -> H/8 (4x) -> H (8x).
    """
    def __init__(self):
        super().__init__()
...
class PatchDecoder_preps(nn.Module):
    """
    Pixel shuffle decoder: H/32 -> H/16 -> H/8 -> H (2x -> 2x -> 8x).
    """
    def __init__(self):
        super().__init__()
...
class ConvDecoder(nn.Module):
    """
    Conv decoder: H/32 -> H/16 -> H/8 -> H (2x -> 2x -> 8x).
    """
