...
class FlowMatchingHead(nn.Module):
    """
    Wrapper for diffusion head in flow matching setup.

    Provides a unified interface for forward pass with timestep conditioning.
    """

    def __init__(self, input_dim, out_dim, dim=1536, layers=12, mlp_ratio=1.0):
        """Initialize flow matching head.

        Args:
            input_dim: Ouro hidden size dimension
            out_dim: VE patch dimension
            dim: Model channel dimension
            layers: Number of residual blocks
            mlp_ratio: MLP expansion ratio
        """
        super().__init__()
        self.net = DiffusionHead(
            input_dim=input_dim,
            out_dim=out_dim,
            dim=dim,
    ...
    def forward(self, x, t):
        ...
        Args:
            x: Backbone hidden states
            t: Timesteps for conditioning

        Returns:
            Predicted clean image features
        """
        return self.net(x, t)


def compute_velocity(x_pred, x_t, t):
    """
    Compute velocity for flow matching.

    In flow matching, the velocity v is computed as:
        v = (x_pred - x_t) / (1 - t)

    where x_pred is the predicted clean sample, x_t is the noisy sample at time t,
    and t is the interpolation time.

    Args:
        x_pred: Predicted clean sample
        x_t: Noisy sample at time t
        t: Interpolation time (0 = noise, 1 = clean)

    Returns:
        Velocity tensor for flow matching loss
    """
    # Avoid division by zero at t=1
    denominator = 1 - t
    # Clamp to prevent extreme values
    denominator = denominator.clamp(min=1e-6)
    velocity = (x_pred - x_t) / denominator
    return velocity